...
test("holding the pen opens persistent nib and color options", function()
    local nb = notebookNamed("notes")
    ...
    assertEq(nb.canvas.pen_color, 255, "white selection")
    local reopened = notebookNamed("notes")
    assertEq(reopened.canvas.pen_style, "fountain", "nib not persisted")
    assertEq(reopened.canvas.pen_color, 255, "color not persisted")
end)
